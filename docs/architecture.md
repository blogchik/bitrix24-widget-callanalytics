...
│   │   │   └── crm_dirty.py               # the dirty queue: mark, due, seq-guarded consume, back-off and hold
│   │   ├── sync/
│   │   │   ├── throttle.py                # per-portal pacing, backoff, Retry-After, operating-time guard (floored)
│   │   │   ├── budgets.py                 # sync_method_budgets: the same guard per (portal, method), statistics excepted
...
│   │   │   ├── crm_dirty_refresh.py       # re-read queued ids, confirm absences, delete behind two guards
│   │   │   ├── crm_deletes.py             # tombstones (decision 29) and unreadable marks
│   │   │   ├── crm_reconcile.py           # daily count-and-split comparison; marks ids, deletes nothing
...
-- ===================== 0003_sync_method_budgets =====================
-- ---------- sync_method_budgets: operating-time state per (portal, method), statistics excepted ----------
CREATE TABLE sync_method_budgets (
    portal_id            bigint      NOT NULL REFERENCES portals(id) ON DELETE CASCADE,
    method               varchar(64) NOT NULL,
    operating_seconds    numeric(8,2),      -- the method's own time.operating accumulator, last seen
    operating_reset_at   timestamptz,
    blocked_until        timestamptz,       -- soft-limit stop or 429 on THIS method
    limit_s              numeric(8,2),      -- learned limit; NULL = default 480
    throttle_hits        integer     NOT NULL DEFAULT 0,
    clean_visits         smallint    NOT NULL DEFAULT 0,
    updated_at           timestamptz NOT NULL DEFAULT now(),
...
### 4.8 CRM tab reads
`GET /api/v1/calls` with `ent` in the JWT loads `crm_contexts(portal_id, ent.t, ent.id)`; the row must have `resolved_at >= JWT.iat` (or `resolved_by_user_id = sub`), otherwise 409 `context_missing` and the SPA runs the session exchange, which re-resolves with the current user's token. Matching: `(crm_entity_type, crm_entity_id) IN entity_keys` **OR** `crm_activity_id = ANY(activity_ids)` **OR** `(crm_entity_type = ent.t AND crm_entity_id = ent.id)` — the last clause covers portals that do emit `DEAL` (or other undocumented types) in the statistics rows. A call made while the client was still a lead keeps `CRM_ENTITY_TYPE = LEAD` forever, and Bitrix24 re-binds its activity to the deal on conversion but leaves the lead as the owner `crm.activity.list` filters on; without a lead key such calls reached no card at all - measured on the first Simple-CRM portal as 1 037 calls missing from 639 deal tabs and 878 from 546 contact tabs (2026-09-26). So a deal's `entity_keys` carry the lead it came from (`LEAD_ID` of `crm.deal.get`, which every opener gets alike), and a contact or company tab adds, at read time, the leads the CRM mirror links to it (`crm_repo.linked_lead_ids`, `contact_ids` / `company_id`, capped at 250). Deliberately not a `crm.lead.list` on the opener's token: the context row is shared between viewers, and keys that depended on the opener's lead rights would make the tab's numbers depend on who opened it last. A portal that does not mirror CRM gets no contact/company lead keys. Then `scope_filter`, then period/paging. `entity_keys` and `activity_ids` always come from the open-time REST read, under the mirror too; only the contact/company lead keys come from `crm_items`.
...
**Leads are optional; deals are not.** A portal whose lead leg answers `MethodNotFound` (leads
turned off) DEGRADES the report to deals-only, sets `scan.leads.available=false` and prints one
sentence; it never refuses. A portal running Bitrix24 **Simple CRM** does NOT answer that way:
its lead API works, but every new lead is converted into a deal at once, with its UTM tags. So
the lead leg is skipped by decision there - `crm.settings.mode.get` (2 = Simple), read by the
dictionary lane into `portals.capabilities.crm_bitrix_mode` - and `scan.leads.reason` is
`simple_crm`; the page draws deals and their win rate and says why (added 2026-09-26). `leads.available=false` is deliberately
...
8. **Rollout is per portal and reversible.** `python -m app.tools.crm_mode` moves a portal `off → sync → shadow → mirror`.
   ...
   - Built in M4b: the switch is `POST /api/v1/portal/crm-analytics` (admin only). Off stamps `crm_opt_out_at`, raises the fence (`sync_generation + 1`) and queues `crm_purge_pending`; the tick's purge slot then runs `sync/purge.py::purge_crm_data`, which deletes `CRM_TENANT_TABLES` under tenant context with the same three verification rules as the uninstall purge and keeps calls. On clears the stamp; storage resumes only once the purge has finished. `/me.crm` carries `analytics_enabled` and `bitrix_mode` (`simple` | `classic`, the portal's own CRM mode) for every viewer and `notice_visible` for administrators, and `python -m app.tools.crm_mode` is the operator's view and mode switch.
   - **Bitrix24 Simple CRM** (added 2026-09-26). The dictionary lane reads `crm.settings.mode.get` in its first batch (no request of its own) into `portals.capabilities.crm_bitrix_mode`, every pass, because portals switch modes; it runs before it is due while no mode is stored, and a failed read never replaces a stored mode. `crm_repo.simple_crm` reads it. On Simple CRM: the Sources report skips the lead leg in both paths (`scan.leads.reason = simple_crm`) and its coverage ignores the lead lanes; `GET /calls` adds `crm.deal_id`, the newest live mirrored deal whose `lead_id` is a LEAD-bound call's lead, and the SPA opens that deal instead of the hidden lead card. `capabilities.crm_bitrix_mode` holds it (0 unknown, 1 Classic, 2 Simple); credential re-stores and the install's bind write keep it. The mode is only read while the CRM lanes run (`crm_mode` not `off`, no opt-out, fleet flag on): a portal that never ran them has none stored and reads as Classic (so a live-path Simple portal keeps the lead leg), and a value stored earlier is kept until the lanes run again. Side reads of the mirror - a tab's contact/company leads, a call's converted deal - happen only while it holds live data (`crm_repo.mirror_readable`). The Deals report's coverage ignores the lead lanes on every portal: it reads no leads.
...
- **Per method** (0003): the guard above is the statistics method's. Every other method the worker calls (`user.get`, `app.info` now; the CRM mirror's methods next) has a `sync_method_budgets` row run through the same rules — soft limit, 429 attribution and limit learning, recovery after `CLEAN_VISITS_TO_RECOVER` clean visits. A soft limit or a 429 on such a method sets its `blocked_until` and skips that method's phases; it never parks the portal, never halves `batch_pages`, and never touches `throttle_hits` of the call sync. A 503 stays portal-wide: the leaky bucket is per source IP.
...
- **Built so far** (milestones M4b and M7), run in `jobs/definitions.py` after the statistics phases and before the employee refresh, so assignees get names in the same visit:
  ...
  - `dirty`;
  - `deal.backfill` / `lead.backfill`;
  - `deal.reconcile` / `lead.reconcile`, daily, once the backfill is done;
  - `deal.patrol` / `lead.patrol`, the backfill walk restarted weekly, first due a cycle after the backfill;
  - the daily `purge_crm_retention`.

  Failures stay on their lane:
  - a lane refused by the portal (CRM or leads off, no such method) retries daily;
  - a fault backs off 60 s doubling to an hour, then six hours after ten;
  - a lane past its share of the method's operating limit rests until the reset;
  - a code defect is caught per lane and never reaches `portal_sync.consecutive_failures`.

  `window`, `history` and `signals` arrive with their milestones.
