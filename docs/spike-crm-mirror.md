...
| S-A.3 | `>=updatedTime` filter; the three-leg OR; `order {updatedTime, id}` | `>=updatedTime` is honoured on deals and leads: every row is at or after the bound, and a year-2999 bound returns 0 rows. The three-leg OR (`createdTime` ∨ `updatedTime` ∨ `closed=Y ∧ movedTime`) is honoured: every row matches a leg. `order {updatedTime:ASC, id:ASC}` is accepted and sorted. Legacy `>=DATE_MODIFY` is honoured. | The sweep and the window bootstrap can filter on `updatedTime` as planned. An `(updatedTime, id)` keyset is available as well as the id keyset. Corrected by S-A.10: the bound is honoured, but read shifted by the token user's zone. |
...
| S-A.10 | How a datetime filter value is read (measured 2026-09-15, after both production sweeps parked on their first page) | The same instant written with `+00:00`, with microseconds, or with `+03:00` returns identical rows, so the offset is parsed. The effective bound is still **two hours earlier than the value**: a record with a known `updatedTime` matched at bounds up to +120 min and stopped matching at +125 min. The installer's profile has a time zone; `user.current` returns no `TIME_ZONE_OFFSET`. A value with no offset is read as wall-clock time and moves further. Rendered timestamps are right: 1 615 leads created from a call carry the call's start time, median difference 0.0 min. | Bitrix24 reads a datetime filter shifted by the token user's zone offset from the server's. The sweep measures the shift per portal and sends its bound moved by it (`sync/crm_clock.py`). East of the server an unmeasured bound over-reads; west of it, it under-reads with no symptom. S-A.3's bounds sat far from the data, where two hours cannot show. The live `/deals` and `/utm` filters, sent on the viewer's token, carry the viewer's shift, so shadow comparisons can differ at a period's edges. |
